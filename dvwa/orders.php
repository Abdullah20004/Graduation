...
if ($orderId > 0) {
    ...
    if ($orderResult && $orderResult->num_rows > 0) {
        ...
        $statusColor = '#fdcb6e'; // Default
        if ($order['status'] == 'confirmed') $statusColor = '#00b894';
        if ($order['status'] == 'shipped') $statusColor = '#0984e3';
        if ($order['status'] == 'delivered') $statusColor = '#6c5ce7';
...
HTML;
    ...
    if (empty($orders)) {
        ...
        foreach ($orders as $o) {
            $statusColor = '#fdcb6e'; 
            if ($o['status'] == 'confirmed') $statusColor = '#00b894';
            if ($o['status'] == 'shipped') $statusColor = '#0984e3';
            if ($o['status'] == 'delivered') $statusColor = '#6c5ce7';
            ...
            $ordersHtml .= <<<HTML
            <a href="/orders.php?id={$o['id']}" class="card" style="display:flex; align-items:center; gap:20px; padding:20px; margin-bottom:12px; text-decoration:none; transition:transform 0.2s, box-shadow 0.2s; cursor:pointer;">
